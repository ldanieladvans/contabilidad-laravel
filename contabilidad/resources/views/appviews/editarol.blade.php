...
@section('variable_content')
	<div class="row">
		...
		<!-- Div contenedor del formulario -->
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				{{ Form::open(['route' => ['roles.update', $rol->id], 'class'=>'form-horizontal form-label-left', 'id'=>'editarol']) }}
					{{ Form::hidden('_method', 'PUT') }}
						...
						<div class="form-group">
							...
							<div class="col-md-10 col-sm-10 col-xs-12">
								<div class="clearfix">
									<input type="text" name="name" id="name" class="col-md-10 col-sm-10 col-xs-12" value="{{ $rol->name }}"/>
								</div>
								@if ($errors->has('name'))
						...
						<div class="form-group">
							...
							<div class="col-md-10 col-sm-10 col-xs-12">
								<div class="clearfix">
									<input type="text" name="slug" id="slug" class="col-md-10 col-sm-10 col-xs-12" value="{{ $rol->slug }}"/>
								</div>
								@if ($errors->has('slug'))
                                    <span class="help-block">
						...
						<div class="form-group">
							...
							<div class="col-md-10 col-sm-10 col-xs-12">
								<div class="clearfix">
									<input type="text" name="description" id="description" class="col-md-10 col-sm-10 col-xs-12" value="{{ $rol->description }}"/>
								</div>
						                    ...
						                    <div class="col-md-10 col-sm-10 col-xs-12">
						                        <select id="permisos" name="permisos[]" tabindex="2" data-placeholder="Seleccione los permisos ..." class="js-example-basic-multiple" multiple="multiple" style="width: 100%; display: none;">
													@foreach($permisos as $permiso)
						                            	<option value="{{ $permiso->id }}" {{Auth::user()->customGetRolePerms($rol->id,$permiso->id,true) > 0 ? 'selected':''}}>{{ $permiso->name }}</option>
						                            @endforeach
						                      </select>
						                  	</div>
							            </div>
