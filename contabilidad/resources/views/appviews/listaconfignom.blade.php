...
@section('variable_content')
	<div class="row">
		<div id="crearconfignom"></div>
		<!-- Input para guardar datos del grid -->
			<input type="hidden" value="{{$confignom}}" id="confignom"/>
        ...
        <script type="text/javascript">
        	...
        	/* DataGrid */
	        	$(function(){
					...
					var data = jQuery.parseJSON(document.getElementById('confignom').value);

					if(data.length > 0){
    					$('#crearconfignom').addClass('hidden');
    				}

				    var dataGrid = $("#confignomLista").dxDataGrid({
				        dataSource: data,
				        ...
				        columns: [
				            {
				                dataField: "confnom_contabiliz_nom",
				                caption: 'Contabilización'
				            },
				            {
				                dataField: "confnom_prov_nom_cta_id",
				                caption: 'Cuenta Provisión'
				            },
				            {
				                dataField: "confnom_percep_cta_id",
				                caption: 'Cuenta Percepción'
				            },
				            {
				                dataField: "confnom_retenc_cta_id",
				                caption: 'Cuenta Retención'
				            },
				            {
				                dataField: "confnom_otrospag_cta_id",
				                caption: 'Cuenta Otros'
				            }
				        ],
				            ...
				            bootbox.confirm("¿Está seguro que quiere eliminar estos registros?", function(result) {
								if(result) {
						    		...
						    		$.ajax({
						                url: '/delItems',
					                	type: 'POST',
					                	data: {_token: CSRF_TOKEN,ids:del_list,model:'ConfigNomina'},
						                dataType: 'JSON',
				    ...
				    var editButton = $("#editarLista").dxButton({
				        ...
				        icon: 'edit',
				        onClick: function () {
				            window.location.href = 'contconfig/'+dataGrid.getSelectedRowKeys()[0]['ID']+'/edit';
				        }
				    }).dxButton("instance");  
				});
        </script>
