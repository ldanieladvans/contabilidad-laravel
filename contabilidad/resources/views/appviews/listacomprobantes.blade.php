	...
	<div class="breadcrumbs ace-save-state" id="breadcrumbs">
	    <ul class="breadcrumb">
	        <li>
	            ...
	            <a href="#">Lista de Comprobantes</a>
	        </li>
	    </ul>
	</div>
@endsection
...
@section('variable_content')
	<div class="row">
		<div id="crearcomprobante"></div>
		<!-- Input para guardar datos del grid -->
			<input type="hidden" value="{{$comprobantes}}" id="comprobantes"/>
        ...
        <script type="text/javascript">
        	...
        	/* DataGrid */
	        	$(function(){
					...
					var data = jQuery.parseJSON(document.getElementById('comprobantes').value);

				    var dataGrid = $("#comprobanteLista").dxDataGrid({
				        dataSource: data,
				        ...
				        columns: [
				            ...
				            {
				                dataField: "comp_rfc_emisor",
				                caption: 'RFC Emisor'
				            },
				            {
				                dataField: "comp_rfc_receptor",
				                caption: 'RFC Receptor'
				            },
				            {
				                dataField: "comp_f_emision",
				                caption: 'Fecha emision'
				            },
				            {
				                dataField: "comp_complmto",
				                caption: 'Complemento'
				            },
				            {
				            ...
				            bootbox.confirm("¿Está seguro que quiere eliminar estos registros?", function(result) {
								if(result) {
						    		...
						    		$.ajax({
						                url: '/delItems',
					                	type: 'POST',
					                	data: {_token: CSRF_TOKEN,ids:del_list,model:'Comprobante'},
						                dataType: 'JSON',
				    ...
				    var editButton = $("#editarLista").dxButton({
				        ...
				        icon: 'edit',
				        onClick: function () {
				            window.location.href = 'comprobantes/'+dataGrid.getSelectedRowKeys()[0]['ID']+'/edit';
				        }
				    }).dxButton("instance");  
				});
        </script>
