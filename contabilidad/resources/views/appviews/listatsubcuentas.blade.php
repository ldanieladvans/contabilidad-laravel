	...
	<div class="breadcrumbs ace-save-state" id="breadcrumbs">
	    <ul class="breadcrumb">
	        <li>
	            ...
	            <a href="#">Lista de Tipos de Sub-Cuenta</a>
	        </li>
	    </ul>
	</div>
@endsection
...
@section('variable_content')
	<div class="row">
		<div id="creartsubcuenta"></div>
		<!-- Input para guardar datos del grid -->
			<input type="hidden" value="{{$tsubcuentas}}" id="tsubcuentas"/>
        ...
        <script type="text/javascript">
        	...
        	/* DataGrid */
	        	$(function(){
					...
					var data = jQuery.parseJSON(document.getElementById('tsubcuentas').value);

				    var dataGrid = $("#tsubcuentaLista").dxDataGrid({
				        dataSource: data,
				        ...
				        columns: [
				            {
				                dataField: "tiposubcta_nom",
				                caption: 'Nombre'
				            },
				            {
				                dataField: "tiposubcta_mostrar_tab",
				                caption: 'Mostrar'
				            }
				        ],
				            ...
				            bootbox.confirm("¿Está seguro que quiere eliminar estos registros?", function(result) {
								if(result) {
						    		...
						    		$.ajax({
						                url: '/delItems',
					                	type: 'POST',
					                	data: {_token: CSRF_TOKEN,ids:del_list,model:'TipoSubCuenta'},
						                dataType: 'JSON',
				    ...
				    var editButton = $("#editarLista").dxButton({
				        ...
				        icon: 'edit',
				        onClick: function () {
				            window.location.href = 'tsubcuentas/'+dataGrid.getSelectedRowKeys()[0]['ID']+'/edit';
				        }
				    }).dxButton("instance");  
				});
        </script>
