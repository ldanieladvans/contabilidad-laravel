	...
	<div class="breadcrumbs ace-save-state" id="breadcrumbs">
	    <ul class="breadcrumb">
	        <li>
	            ...
	            <a href="#">Lista de Pagos Relacionados</a>
	        </li>
	    </ul>
	</div>
@endsection
...
@section('variable_content')
	<div class="row">
		<div id="crearpagorel"></div>
		<!-- Input para guardar datos del grid -->
			<input type="hidden" value="{{$pagosrel}}" id="pagosrel"/>
        ...
        <script type="text/javascript">
        	...
        	/* DataGrid */
	        	$(function(){
					...
					var data = jQuery.parseJSON(document.getElementById('pagosrel').value);

				    var dataGrid = $("#pagorelLista").dxDataGrid({
				        dataSource: data,
				        ...
				        columns: [
				            {
				                dataField: "pagorel_pago_id",
				                caption: 'Pago'
				            },
				            {
				                dataField: "pagorel_pagado_uuid",
				                caption: 'Pago UUID'
				            },
				            {
				                dataField: "pagorel_serie",
				                caption: 'Serie'
				            },
				            {
				                dataField: "pagorel_moneda_nom",
				                caption: 'Moneda'
				            },
				            {
				                dataField: "pagorel_monto_pag",
				                caption: 'Monto'
				            },
				            {
				                dataField: "pagorel_asiento_id",
				                caption: 'Asiento'
				            }
				        ],
				            ...
				            bootbox.confirm("¿Está seguro que quiere eliminar estos registros?", function(result) {
								if(result) {
						    		...
						    		$.ajax({
						                url: '/delItems',
					                	type: 'POST',
					                	data: {_token: CSRF_TOKEN,ids:del_list,model:'Pagorel'},
						                dataType: 'JSON',
				    ...
				    var editButton = $("#editarLista").dxButton({
				        ...
				        icon: 'edit',
				        onClick: function () {
				            window.location.href = 'pagosrel/'+dataGrid.getSelectedRowKeys()[0]['ID']+'/edit';
				        }
				    }).dxButton("instance");  
				});
        </script>
