	...
	<div class="breadcrumbs ace-save-state" id="breadcrumbs">
	    <ul class="breadcrumb">
	        <li>
	            ...
	            <a href="#">Lista de Tipos de Cliente</a>
	        </li>
	    </ul>
	</div>
@endsection
...
@section('variable_content')
	<div class="row">
		<div id="creartcliente"></div>
		<!-- Input para guardar datos del grid -->
			<input type="hidden" value="{{$tclientes}}" id="tclientes"/>
        ...
        <script type="text/javascript">
        	...
        	/* DataGrid */
	        	$(function(){
					...
					var data = jQuery.parseJSON(document.getElementById('tclientes').value);

				    var dataGrid = $("#tclienteLista").dxDataGrid({
				        dataSource: data,
				        ...
				        columns: [
				            {
				                dataField: "tipcliente_desc",
				                caption: 'Nombre/Descripción'
				            },
				            {
				                dataField: "tipcliente_concpto_polz",
				                caption: 'Concepto'
				            }
				        ],
				            ...
				            bootbox.confirm("¿Está seguro que quiere eliminar estos registros?", function(result) {
								if(result) {
						    		...
						    		$.ajax({
						                url: '/delItems',
					                	type: 'POST',
					                	data: {_token: CSRF_TOKEN,ids:del_list,model:'TipoCliente'},
						                dataType: 'JSON',
				    ...
				    var editButton = $("#editarLista").dxButton({
				        ...
				        icon: 'edit',
				        onClick: function () {
				            window.location.href = 'tclientes/'+dataGrid.getSelectedRowKeys()[0]['ID']+'/edit';
				        }
				    }).dxButton("instance");  
				});
        </script>
