	...
	<div class="breadcrumbs ace-save-state" id="breadcrumbs">
	    <ul class="breadcrumb">
	        <li>
	            ...
	            <a href="#">Lista de Tipos de Proveedor</a>
	        </li>
	    </ul>
	</div>
@endsection
...
@section('variable_content')
	<div class="row">
		<div id="creartproveedor"></div>
		<!-- Input para guardar datos del grid -->
			<input type="hidden" value="{{$tproveedores}}" id="tproveedores"/>
        ...
        <script type="text/javascript">
        	...
        	/* DataGrid */
	        	$(function(){
					...
					var data = jQuery.parseJSON(document.getElementById('tproveedores').value);

				    var dataGrid = $("#tproveedorLista").dxDataGrid({
				        dataSource: data,
				        ...
				        columns: [
				            {
				                dataField: "tipprov_desc",
				                caption: 'Nombre/Descripción'
				            },
				            {
				                dataField: "tipprov_concpto_polz",
				                caption: 'Concepto'
				            }
				        ],
				            ...
				            bootbox.confirm("¿Está seguro que quiere eliminar estos registros?", function(result) {
								if(result) {
						    		...
						    		$.ajax({
						                url: '/delItems',
					                	type: 'POST',
					                	data: {_token: CSRF_TOKEN,ids:del_list,model:'TipoProveedor'},
						                dataType: 'JSON',
				    ...
				    var editButton = $("#editarLista").dxButton({
				        ...
				        icon: 'edit',
				        onClick: function () {
				            window.location.href = 'tproveedores/'+dataGrid.getSelectedRowKeys()[0]['ID']+'/edit';
				        }
				    }).dxButton("instance");  
				});
        </script>
